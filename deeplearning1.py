...
def maybe_download(filename, expected_bytes, force=False):
	...
	if statinfo.st_size == expected_bytes:
		...
		raise Exception(
			'Failed to verify ' + filename + '. Can you open with a browser?')
	return filename

train_file = maybe_download('notMNIST_large.tar.gz', 247336696)
test_file = maybe_download('notMNIST_small.tar.gz', 8458043)

num_classes = 10
np.random.seed(133)

def maybe_extract(filename, force=False):
	root = os.path.splitext(os.path.splitext(filename)[0])[0] # this removes the .tar.gz
	if os.path.isdir(root) and not force:
		print('%s already present - Skipping extraction of %s.' % (root, filename))
	else:
		print('Extracting data for %s. This may take a while so be patient.' % root)
...
def load_letter(folder, min_num_images):
	...
	for image_index, image in enumerate(image_files):
		...
		global last_percent_reported
		percent = int(image_index*100/len(image_files))
